...
function gamePiece(x, y, p, player) {
    ...
    this.threat = function() {
        ...
        if (this.rank == "p") {
            ...
            if (this.first && (!this.whereIs(this.Player.Board.players[playerIndex].pieces, this.x, this.y + (playerIndex ? 1 : -1)))
				&& (!this.whereIs(this.Player.Board.players[playerIndex].pieces, this.x, this.y + (playerIndex ? 2 : -2)))) {
                move_list.push(this.moveFilter([[0, (playerIndex ? 1 : -1)], [0, (playerIndex ? 2 : -2)]], this.Player));
            }
            else {
                if (!this.whereIs(this.Player.Board.players[playerIndex].pieces, this.x, this.y + (playerIndex ? 1 : -1))) move_list.push(this.moveFilter([[0, (playerIndex ? 1 : -1)]], this.Player));
            }
        }
        return move_list;
    }
